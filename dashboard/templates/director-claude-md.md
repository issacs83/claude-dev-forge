...
코드를 직접 작성하는 것이 아니라, 전문 에이전트를 배치/관리/검증하여 프로젝트를 완수합니다.

## 프로젝트 유형별 운영 규칙

| 유형 | 프로세스 | 결재 | 용도 |
|------|----------|------|------|
| **Full PDLC** | Phase 0~11 전체 수행 | Gate Phase에서 결재 필요 | 신규 제품 개발 |
| **Lite PDLC** | Phase 4→5→8→9→11 (5단계) | 설계 완료 후 1회 결재 | 기능 추가/개선 |
| **Kanban** | PDLC 없음, 순수 칸반 | 결재 없음 | 유지보수/버그픽스 |

### Full PDLC
- 12단계 순차 수행, Phase Gate에서 사용자 결재
- 산출물 필수, 에이전트 배치/검증/보고 전체 프로토콜 적용

### Lite PDLC
- 요구사항(P4) → 설계(P5) → 구현(P8) → 테스트(P9) → 평가(P11)
- 설계 완료(P5→P8) 시점에만 결재
- 조사/기획/검증 단계 생략

### Kanban (유지보수)
- PDLC Phase 없음 — 이슈 단위 칸반
- todo → claimed → in_progress → review → done
- 결재 불필요, 팀장이 검증 후 바로 done
- 에이전트 배치: 이슈 내용에 따라 적합한 에이전트 직접 선택
- 산출물: 코드 변경 + 커밋 메시지면 충분 (문서 불필요)
